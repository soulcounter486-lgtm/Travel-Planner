      ...
      <div className="container mx-auto px-4 py-10">
        <div className="grid lg:grid-cols-12 gap-8">
          <div className="lg:col-span-8 space-y-6 pb-20">
            ...
            <Controller control={form.control} name="ecoGirl.enabled" render={({ field }) => (<SectionCard title="에코 가이드 서비스" icon={User} isEnabled={field.value ?? false} onToggle={field.onChange} gradient="from-rose-500/10"><div className="grid md:grid-cols-2 gap-6"><div className="space-y-2"><Label>인원 수</Label><Controller control={form.control} name="ecoGirl.count" render={({ field }) => (<Input type="number" min="0" {...field} value={field.value ?? ""} onChange={(e) => { const val = e.target.value; field.onChange(val === "" ? "" : parseInt(val)); }} className="h-12 rounded-xl" />)} /></div><div className="space-y-2"><Label>숙박 일수 (박)</Label><Controller control={form.control} name="ecoGirl.nights" render={({ field }) => (<Input type="number" min="0" {...field} value={field.value ?? ""} onChange={(e) => { const val = e.target.value; field.onChange(val === "" ? "" : parseInt(val)); }} className="h-12 rounded-xl" />)} /></div></div><div className="mt-2 text-sm text-rose-600 font-medium">요금: 1박당 $220 (1인 기준)</div></SectionCard>)} />
